...
--
-- Client :  localhost
-- Généré le :  Dim 25 Novembre 2018 à 01:44
...
(14, 'Le gouvernement ne devrait pas tarder', 'Voil?  un secteur qui, bien qualertant r?©guli??rement sur ses marges de plus en plus r?©duites, ne semble pas conna?®tre la crise. La France  l???un des pays d???Europe comptant d?©j?  le plus d???hypermarch?©s par habitant ??? devrait totaliser 896 zones commerciales ?  l???horizon 2021, dont 72 flambant neuves et plus d???une centaine r?©nov?©es au cours des cinq derni??res ann?©es (voir chiffres-cl?©s). Au grand dam de certains ?©lus qui associent d?©veloppement anarchique de la grande distribution et crise urbaine ??? d?©vitalisation des centres-villes, vacance commerciale et/ou r?©sidentielle, etc.'),
...
--
ALTER TABLE `articles`
  MODIFY `id_article` int(32) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=27;
